package Vectors is
   ...
   function "+" (Left : Vector; Right : Vector) return Vector with
      Post =>
      ("+"'Result.X = Left.X + Right.X
       and then "+"'Result.Y = Left.Y + Right.Y
       and then "+"'Result.Z = Left.Z + Right.Z);
   --  PRE
   --  prevent overflow (MISSING)
   function "-" (Left : Vector; Right : Vector) return Vector with
      Post =>
      ("-"'Result.X = Left.X - Right.X
       and then "-"'Result.Y = Left.Y - Right.Y
   ...
   function "*" (Left : Vector; Right : Float) return Vector with
      Post =>
      ("*"'Result.X = Left.X * Right
       and then "*"'Result.Y = Left.Y * Right
       and then "*"'Result.Z = Left.Z * Right);
      --  Multiplies all dimensions of Left by Right.
   function "*" (Left : Vector; Right : Vector) return Float with
      Post =>
      ("*"'Result = Left.X * Right.X + Left.Y * Right.Y + Left.Z * Right.Z);
      --  Computes the scalar product.
   overriding function "=" (Left : Vector; Right : Vector) return Boolean with
      Post =>
       (if Left.X = Right.X
   ...
   function Are_Orthogonal (Left : Vector; Right : Vector) return Boolean with
      Post =>
      (if Left * Right = 0.0 then Are_Orthogonal'Result
       else Are_Orthogonal'Result = False);

      --  Determines if both vectors stand orthogonal to each other or not.
   function Cross_Product (Left : Vector; Right : Vector) return Vector with
      Post =>
      (Cross_Product'Result.X = Left.Y * Right.Z + Left.Z * Right.Y
